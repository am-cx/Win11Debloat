...
# 如果当前的 Powershell 环境受到安全策略的限制，则显示错误
...
function RemoveApps {
    ...
    Foreach ($app in $appsList) { 
        ...
        try {
            ...
            Write-Host "无法从 windows 图像中删除 $app" -ForegroundColor Yellow
            Write-Host $psitem.Exception.StackTrace -ForegroundColor Gray
        }
    }
...
function RestartExplorer {
    ...
    if ($script:Params.ContainsKey("禁用鼠标加速")) {
        Write-Host "警告: 增强指针精度设置更改仅在重新启动后生效" -ForegroundColor Yellow
    }

    if ($script:Params.ContainsKey("禁用 StickyKeys")) {
        Write-Host "警告: 粘性键设置更改只有在重新启动后才会生效" -ForegroundColor Yellow
    }

    if ($script:Params.ContainsKey("禁用动画")) {
        Write-Host "警告: 只有在重启后才会禁用动画" -ForegroundColor Yellow
    }

    # 仅当 powershell 进程与操作系统架构匹配时，才重新启动.
    # Restarting explorer from a 32bit PowerShell window will fail on a 64bit OS
    if ([Environment]::Is64BitProcess -eq [Environment]::Is64BitOperatingSystem) {
        Stop-Process -processName: Explorer -Force
    }
    else {
        Write-Warning "无法重新启动  Windows 资源管理器进程, 请手动重启 PC 以应用所有更改."
    }
}
...
function ReplaceStartMenuForAllUsers {
    ...
    # 也替换默认用户配置文件的开始菜单文件
    $defaultStartMenuPath = GetUserDirectory -userName "默认选项" -fileName "AppData\Local\Packages\Microsoft.Windows.StartMenuExperienceHost_cw5n1h2txyewy\LocalState" -exitIfPathNotFound $false
...
function CreateSystemRestorePoint {
    ...
    $createRestorePointJob = Start-Job { 
        ...
        try {
            ...
            Write-Host "错误: 无法检索现有的恢复点: $_" -ForegroundColor Red
            return
        }
    
        if ($recentRestorePoints.Count -eq 0) {
            try {
                Checkpoint-Computer -Description "由 Win11Debloat 创建的恢复点" -RestorePointType "修改_设置"
...
function ShowDefaultMode {
    AddParameter '创建还原点' '创建系统还原点' $false
    ...
    elseif ($RunDefaultsLite) {
        ...
        if (($script:selectedApps.contains('Microsoft.Xbox 游戏覆盖') -or $script:selectedApps.contains('Microsoft.XboxGamingOverlay')) -and 
          $( Read-Host -Prompt "禁用游戏栏集成和游戏/屏幕录制？ 这也阻止了 ms-gamingoverlay 和 ms-gamebar 弹出窗口 (y/n)" ) -eq 'y') {
    ...
    switch ($RemoveAppsInput) {
        '1' {
            AddParameter 'RemoveApps' '删除应用程序的默认选择:' $false
            PrintAppsList "$PSScriptRoot/Appslist.txt"
        }
        '2' {
            AddParameter '删除应用程序自定义' "Remove $($script:SelectedApps.Count) apps:" $false
            PrintAppsList "$PSScriptRoot/CustomAppsList"
        }
    }

    if ($DisableGameBarIntegrationInput) {
        AddParameter '禁用 DVR' '停用Xbox游戏/屏幕录制' $false
        AddParameter '禁用 GameBar 集成' '禁用游戏栏集成' $false
    }

    # 仅为 Windows 10 用户添加此选项
    if (get-ciminstance -query "select caption from win32_operatingsystem where caption like '%Windows 10%'") {
        AddParameter 'Hide3dObjects' "在文件资源管理器中隐藏“此电脑”下的3D对象文件夹" $false
        AddParameter '隐藏聊天' '从任务栏中隐藏聊天（立即见面）图标' $false
    }

    # 仅为 Windows 11 用户添加这些选项  (build 22000+)
    if ($WinVersion -ge 22000) {
        if ($script:ModernStandbySupported) {
            AddParameter '禁用现代待机网络' '在现代待机期间禁用网络连接' $false
        }

        AddParameter '禁用召回' '停用 Windows 召回' $false
        AddParameter '禁用点击待办事项' '禁用点击做（人工智能文本和图像分析)' $false
    }
    ...
    $DefaultParameterNames = '禁用副驾驶','禁用遥测','禁用建议','停用 EdgeAds','禁用锁定屏幕提示','禁用Bing','显示已知文件扩展','禁用小部件','禁用快速启动'
...
function ShowDefaultModeAppRemovalOptions {
    ...
    Write-Host "请注意：默认选择的应用程序包括微软团队、Spotify、便笺等。 选择选项 2 来验证并更改脚本删除的应用程序." -ForegroundColor DarkGray
    Write-Host ""
...
function ShowCustomModeOptions {
    ...
    AddParameter 'CreateRestorePoint' '创建系统恢复点'
    ...
    switch ($RemoveAppsInput) {
        '1' {
            AddParameter 'RemoveApps''删除应用程序的默认选择'
        }
        '2' {
            ...
            AddParameter 'DisableDVR' '停用 Xbox 游戏/屏幕录制'
            AddParameter 'DisableGameBar 集成' '禁用游戏栏集成'
        }
        '3' {
            ...
            if ($( Read-Host -Prompt "禁用游戏栏集成和游戏/屏幕录制？ 这也阻止了 ms-gamingoverlay 和 ms-gamebar 弹出窗口 (y/n)" ) -eq 'y') {
                AddParameter '禁用 DVR''禁用 Xbox 游戏/屏幕录制'
                AddParameter '禁用 GameBar 集成' '禁用游戏栏集成'
            }
        }
    }
    ...
    if ($( Read-Host -Prompt "禁用遥测、诊断数据、活动历史记录、应用程序启动跟踪和定向广告? (y/n)" ) -eq 'y') {
        AddParameter '禁用遥测''禁用遥测、诊断数据、活动历史记录、应用程序启动跟踪和定向广告'
    }

    Write-Output ""

    if ($( Read-Host -Prompt "在开始、设置、通知、资源管理器、锁定屏幕和 Edge? (y/n)" ) -eq 'y') {
        AddParameter '禁用建议''在开始、设置、通知和文件资源管理器中禁用提示、技巧、建议和广告'
        AddParameter '停用EdgeAds'在Microsoft Edge中禁用广告、建议和MSN新闻提要'
        AddParameter '禁用Settings365Ads''在设置主页中禁用Microsoft 365广告'
        AddParameter '禁用锁屏提示“禁用锁屏上的提示和窍门”'
    }
    ...
    if ($WinVersion -ge 22621) {
        ...
        Do {
            ...
            Write-Host " (1) 禁用 Microsoft Copilot、Windows 召回和点击操作" -ForegroundColor Yellow
            Write-Host " (2) 在 Microsoft Edge、Paint 和 Notepad 中禁用 Microsoft Copilot、Windows Recall、Click to Do和AI功能"  -ForegroundColor Yellow
        ...
        switch ($DisableAIInput) {
            '1' {
                AddParameter '禁用副驾驶' '禁用并删除Microsoft Copilot'
                AddParameter '禁用召回' '停用 Windows 召回'
                AddParameter '禁用待办事项' '禁用点击做（人工智能文本和图像分析)'
            }
            '2' {
                AddParameter '禁用副驾驶' '禁用并删除 Microsoft Copilot'
                AddParameter 'DisableRecall' '停用Windows召回'
                AddParameter '禁用 ClickToDo' '禁用点击做（人工智能文本和图像分析)'
                AddParameter '禁用 EdgeAI' '禁用人工智能功能 Edge'
                AddParameter '禁用 PaintAI' '在Paint中禁用AI功能'
                AddParameter '禁用 NotepadAI' '禁用记事本中的人工智能功能'
            }
        }
    }
    ...
    if ($( Read-Host -Prompt "在桌面上禁用 Windows 聚光灯背景? (y/n)" ) -eq 'y') {
    ...
    if ($( Read-Host -Prompt "为系统和应用程序启用深色模式? (y/n)" ) -eq 'y') {
        AddParameter '启用暗模式''为系统和应用程序启用暗模式'
    }

    Write-Output ""

    if ($( Read-Host -Prompt "禁用透明度、动画和视觉效果? (y/n)" ) -eq 'y') {
        AddParameter '禁用透明度''禁用透明度效果'
        AddParameter '禁用动画“禁用动画和视觉效果”'
    }
    ...
    if ($WinVersion -ge 22000) {
        ...
        if ($( Read-Host -Prompt "恢复旧的 Windows 10 样式上下文菜单? (y/n)" ) -eq 'y') {
            AddParameter '恢复上下文菜单' '恢复旧的Windows 10样式上下文菜单'
        }
    }
    ...
    if ($( Read-Host -Prompt "关闭增强指针精度，也称为鼠标加速? (y/n)" ) -eq 'y') {
        AddParameter '禁用鼠标加速' '关闭增强指针精度（鼠标加速)'
    }
    ...
    if (($WinVersion -ge 22000) -and $script:ModernStandbySupported) {
        ...
        if ($( Read-Host -Prompt "在现代待机期间禁用网络连接？ 这适用于所有用户 (y/n)" ) -eq 'y') {
            AddParameter '禁用现代待机网络' '在现代待机期间禁用网络连接'
        }
    }

    # 仅显示为Windows 10用户禁用上下文菜单项的选项，或者如果用户选择恢复 Windows 10 上下文菜单
        ...
        if ($( Read-Host -Prompt "你想对开始菜单进行任何更改吗? (y/n)" ) -eq 'y') {
            ...
            if ($script:Params.ContainsKey("Sysprep")) {
                ...
                switch ($ClearStartInput) {
                    ...
                    '2' {
                        AddParameter '清除开始所有用户' "从所有现有和新用户的开始菜单中删除所有固定的应用程序"
                    }
                }
            }
...
function ShowAppRemoval {
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
        Write-Output "您已选择 $($script:SelectedApps.Count) 要删除的应用程序"
        AddParameter '删除应用程序自定义' "移除 $($script:SelectedApps.Count) 应用程序:"

        # 如果静音参数已通过，请抑制提示
        if (-not $Silent) {
            Write-Output ""
            Write-Output ""
            Write-Output "按回车键删除选定的应用程序，或按 CTRL+C 退出..."
            Read-Host | Out-Null
            PrintHeader "应用程序移除"
        }
    }
    else {
        Write-Host "选择已取消，没有删除任何应用程序" -ForegroundColor Red
        Write-Output ""
    }
}
...
if ((Get-AppxPackage -Name "*Microsoft.DesktopAppInstaller*") -and ([int](((winget -v) -replace 'v','').split('.')[0..1] -join '') -gt 14)) {
    ...
    if (-not $Silent) {
        Write-Warning "Winget 未安装或过时。 这可能会阻止 Win11Debloat 删除某些应用程序."
        Write-Output ""
        Write-Output "无论如何，按任意键继续..."
        $null = [System.Console]::ReadKey()
    }
}
...
$SPParams = “如果”、“确认”、“动词”、“无语”、“Sysprep”、“调试”、“用户”、“创建恢复点”、“日志路径”
$SPParamCount = 0
...
if ($script:Params.ContainsKey("用户")) {
    $userPath = GetUserDirectory -userName $script:Params.Item("用户")
}
...
if ($RunAppConfigurator -or $RunAppsListGenerator) {
    ...
    if ($result -ne [System.Windows.Forms.DialogResult]::OK) {
        Write-Host "应用程序选择窗口在未保存的情况下关闭." -ForegroundColor Red
    }
    else {
...
if ((-not $script:Params.Count) -or $RunDefaults -or $RunDefaultsLite -or $RunSavedSettings -or ($SPParamCount -eq $script:Params.Count)) {
    ...
    elseif ($RunSavedSettings) {
        if (-not (Test-Path "$PSScriptRoot/SavedSettings")) {
            PrintHeader 'Custom Mode'
            Write-Host "错误：未找到已保存的设置，未进行更改" -ForegroundColor Red
            AwaitKeyToExit
        }
...
switch ($script:Params.Keys) {
    ...
    'RemoveAppsCustom' {
        ...
        Write-Output "> 移除 $($appsList.Count) 应用程序..."
        RemoveApps $appsList
        continue
    }
    'RemoveCommApps' {
        $appsList = 'Microsoft.windows 通信应用程序', 'Microsoft.People'
        Write-Output "> 移除“邮件”、“日历”和“人员”应用..."
        RemoveApps $appsList
        continue
    }
    'RemoveW11Outlook' {
        $appsList = 'Microsoft.Outlook 适用于 Windows'
